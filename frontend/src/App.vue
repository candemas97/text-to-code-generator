    ...
    <section class="container">
        <header>
            ...
            <section class="language">
                <span :class="{ 'language-label': true, 'selected-language': selectedLanguageApp === 'english' }" @click="setSelectedLanguageApp('english')"> EN </span>
                <span class="dot">•</span>
                <span :class="{ 'language-label': true, 'selected-language': selectedLanguageApp === 'spanish' }" @click="setSelectedLanguageApp('spanish')"> ES </span>
            </section>
        </header>
        <section class="container-2">
            <section class="container-section">
                <textarea v-model="query" spellcheck="false" placeholder="... cuál es tu prompt?" />
                <span class="microphone-container">
                    ...
                    <MicrophoneIcon class="microphone-icon" @click="startRecording(selectedLanguageApp)" />
                </span>
            </section>
            <div class="generating" v-if="isFetching">Generando...</div>
            <button @click="requestModel" v-if="!isFetching">Execute</button>
            <pre v-if="!isFetching && response.length > 0">
                <code class="language-python">{{ response }}</code>
            </pre>
        </section>
        <!-- <Speech /> -->
        <footer>By @yutatis, @herrera, @candemas ❤️</footer>
    </section>
</template>
...
export default defineComponent({
    name: "App",
    components: { MicrophoneIcon, Speech },
    data: function () {
        return {
            query: "",
            response: "",
